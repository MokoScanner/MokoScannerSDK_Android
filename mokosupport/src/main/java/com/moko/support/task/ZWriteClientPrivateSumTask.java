...
public class ZWriteClientPrivateSumTask extends OrderTask {
    private static final int ORDERDATA_LENGTH = 5;

    private byte[] orderData;

    public ZWriteClientPrivateSumTask(MokoOrderTaskCallback callback, int dataLength) {
        super(OrderType.CHARACTERISTIC, OrderEnum.WRITE_CLIENTPRIVATE_PACKAGE_SUM, callback, OrderTask.RESPONSE_TYPE_WRITE_NO_RESPONSE);
        int packetCount;
        if (dataLength % 16 > 0) {
