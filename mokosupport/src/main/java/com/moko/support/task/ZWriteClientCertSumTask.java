...
public class ZWriteClientCertSumTask extends OrderTask {
    private static final int ORDERDATA_LENGTH = 5;

    private byte[] orderData;

    public ZWriteClientCertSumTask(MokoOrderTaskCallback callback, int dataLength) {
        super(OrderType.CHARACTERISTIC, OrderEnum.WRITE_CLIENTCERT_PACKAGE_SUM, callback, OrderTask.RESPONSE_TYPE_WRITE_NO_RESPONSE);
        int packetCount;
        if (dataLength % 16 > 0) {
