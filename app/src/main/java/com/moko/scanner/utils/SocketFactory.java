...
public class SocketFactory extends javax.net.ssl.SSLSocketFactory {
    ...
    public SocketFactory(SocketFactoryOptions options) throws KeyStoreException, NoSuchAlgorithmException, IOException, KeyManagementException, java.security.cert.CertificateException, UnrecoverableKeyException {
        ...
        if (options.hasCaCrt()) {
            ...
            LogModule.v(String.format("Certificate Owner: %s", ca.getSubjectDN().toString()));
            LogModule.v(String.format("Certificate Issuer: %s", ca.getIssuerDN().toString()));
            LogModule.v(String.format("Certificate Serial Number: %s", ca.getSerialNumber().toString()));
            LogModule.v(String.format("Certificate Algorithm: %s", ca.getSigAlgName()));
            LogModule.v(String.format("Certificate Version: %s", ca.getVersion()));
            LogModule.v(String.format("Certificate OID: %s", ca.getSigAlgOID()));
            Enumeration<String> aliasesCA = caKeyStore.aliases();
            for (; aliasesCA.hasMoreElements(); ) {
                String o = aliasesCA.nextElement();
                LogModule.v(String.format("Alias: %s isKeyEntry:%s isCertificateEntry:%s", o, caKeyStore.isKeyEntry(o), caKeyStore.isCertificateEntry(o)));
            }


        } else {
            LogModule.v("CA sideload: false, using system keystore");
        ...
        if (options.hasClientP12Crt()) {
            ...
            for (; aliasesClientCert.hasMoreElements(); ) {
                ...
                LogModule.v(String.format("Alias: %s", o));
            }
        } else {
